...
$matchedRole = $_SESSION['pickedRole']; 
// the id of the person that was clicked 
$matchedId = $_SESSION["pickedId"];
		...
		// if ($role == '2') { 
	      ...
	      // var_dump($matches);  
			...
			if ($matchedRole == '1' && $index="f") { // Food
					echo(" delete ");
					$stmt2 = $pdo->prepare("DELETE FROM `Food` WHERE `item` = '$item' AND `id`='$matchedId' ");
					$i = $stmt2->execute();
			...
			if ($matchedRole == '2' && $index == "f") {
				echo(" delete 2 ");
				$stmt2 = $pdo->prepare("DELETE FROM `FoodNeeds` WHERE `item` = '$item' AND id='$id'");
					$i = $stmt2->execute();
