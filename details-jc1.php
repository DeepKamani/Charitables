...
$stmt = $pdo->prepare("SELECT `fname`, `lname`, `organization_name`, `address`, `city`, `state`, `postal_code`, `role` 
FROM `Users` 
WHERE `id` = '$id' ");
$stmt->execute();
$row = $stmt->fetch();
$chosenRole = $row['role'];
$_SESSION['pickedRole'] = $chosenRole;
?>
            ...
            <div class="list-group-horizontal">
                ...
                <form action="process_offers.php" method="POST">
                    ...
                    <div class="container list-group-item list-group-item-action">
                        <?php
                        if ($chosenRole == '1') { // Donor 
                            getDonorDetails($pdo, $id);
                        } else {
                            getRecipientDetails($pdo, $id);
                        }
                        ?>
                    </div>
                    <div class="list-group-item text-center">
                        <p><?php echo ($row['address']) ?><br /><?php echo ($row['city']) ?> <?php echo ($row['state']) ?> <?php echo ($row['postal_code']) ?></p>
                    </div>
...
function getDonorDetails($pdo, $id)
{
    // get the donors food 
    $d1 = $pdo->prepare("SELECT `type`, `foodImg`.`category`, `item`, `quantity`
    FROM `Food` AS food INNER JOIN `FoodImages` AS foodImg
    ON `food`.`category` = `foodImg`.`id`
    WHERE `food`.`id` = '$id' ORDER BY `food`.`category` ");
    $d1->execute();
    // get the donors clothes
    $d2 = $pdo->prepare("SELECT `type`, `clothesImg`.`category`, `item`, `quantity`
...
if ($category != $row['category']) {
    ...
    <div class="col">
        <div class="form-check checkbox-md">
            <input type="checkbox" class="form-check-input" id="<?php echo ($row['item']) ?>" value="<?php echo ($row['item']) ?> <?php echo ($row['type']) ?>" name="requests[]">
